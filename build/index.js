...
function matchHotspot(lat, lng) {
    let best = null;
    for (const h of SEOUL_HOTSPOTS) {
        const d = haversineM(lat, lng, h.lat, h.lng);
        if (!best || d < best.dist)
            best = { name: h.name, dist: d };
    }
    if (best && best.dist <= HOTSPOT_MATCH_RADIUS_M)
        return best.name;
    return null;
}
// 지역명으로 실시간 혼잡도 + 예측 조회. 실패/미지원 지역이면 null 폴백.
async function fetchAreaCongestion(areaNm) {
    ...
    try {
        ...
        if (!row || !row.AREA_CONGEST_LVL)
            return null;
        // 요청 지역과 응답 지역이 다르면(sample키 치환 등) 엉뚱한 지역 혼잡도이므로 버림.
        if (row.AREA_NM !== areaNm)
            return null;
        return {
...
async function fetchDriveETA(origin, dest) {
    if (!KAKAO_REST_API_KEY)
        return null;
    const controller = new AbortController();
...
function createServer() {
    ...
    server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
        ...
        if (name === 'calculate_optimal_midpoint') {
            ...
            const geocoded = await Promise.all(locations.map(async (loc) => ({ loc, coord: await geocode(loc) })));
            const valid = geocoded.filter((g) => g.coord !== null);
            if (valid.length === 0) {
            ...
            const settled = await Promise.allSettled(valid.map((g) => fetchDriveETA(g.coord, destCoord)));
